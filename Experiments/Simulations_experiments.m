...
clear;close all;clc;
ro_grid = [1, 0.5, 0];
for loop_iter=1:numel(ro_grid)

    clearvars -except ro_grid loop_iter  
    rollover = ro_grid(loop_iter);

    str = num2str(rollover);
    str = strcat('Policies\GR_ro_', str,'.mat');
    load(str)
  ...
  % Set initial conditions
    ...
    str = num2str(rollover);
    str = strcat('Simulations\simulations_gr_', str,'.mat');
    save(str)
end
...
clear;close all;clc;
dc_grid = [0.3 0.4 0.5];
for loop_iter=1:numel(dc_grid)

    clearvars -except dc_grid loop_iter   
    dc = dc_grid(loop_iter);
  ...
  % Set initial conditions
    ...
    str = num2str(dc);
    str = strcat('Simulations\simulations_dc_', str,'.mat');
    save(str)
end



%% Simulations beta = 0.86
clear;close all;clc;
load('Policies\full_commit_beta_86.mat')
...
for j=1:Nsim
    ...
    y_sim(:,j) = zeta*z_sim(:,j).*(k_sim(:,j).^alpha).*l_sim(:,j).^(1-alpha);
    b_y(:,j) = b_sim(:,j)./y_sim(:,j);
    b_y_service(:,j) = (lambda + coupon*(1-lambda))*b_sim(:,j)./y_sim(:,j);
    c_sim(:,j) = y_sim(:,j)./(1+tau);
    tax_sim(:,j) = tau*c_sim(:,j);
    i_sim(2:end,j) = k_sim(2:end,j) - (1-delta).*k_sim(1:end-1,j);
    rr = (lambda +(1-lambda)*coupon)./q - lambda;
    spread(:,j) = (1+rr).^4-(1+r).^4;
    u_sim(:,j) = (1-omega).*(c_sim(:,j) - l_sim(:,j).^(1+psi)./(1+psi)).^(1-sigma)./(1-sigma) + ...
        omega.*g_sim(:,j).^(1-sigma)./(1-sigma);
end
...
[YHPT, YHP] = hpfilter(y_sim, 1600);
[BHPT, BHP] = hpfilter(b_sim, 1600);
[BYserviceHPT, BYserviceHP] = hpfilter(b_y_service, 1600);
[B_YHPT, B_YHP] = hpfilter(b_y, 1600);
[LHPT, LHP] = hpfilter(l_sim, 1600);
[KHPT, KHP] = hpfilter(k_sim, 1600);
[IHPT, IHP] = hpfilter(i_sim, 1600);
...
save('Simulations\simulations_fc_86.mat')


%% Simulations beta = 0.99
clear;close all;clc;
load('Policies\full_commit_beta_99.mat')
...
save('Simulations\simulations_fc_99.mat')